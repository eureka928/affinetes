...
# R2 prefix for expansion tasks generated by affine-swe-infinite
EXPANSION_R2_PREFIX = "expansion"

# Generic pytest output parser for expansion (github-pr) tasks.
# Parses `pytest -v --tb=no` output and writes {"tests": [...]} JSON.
GENERIC_PYTEST_PARSER = r"""import json, re, sys
from pathlib import Path
stdout = Path(sys.argv[1]).read_text()
stderr = Path(sys.argv[2]).read_text()
pat = re.compile(r"^([\w/.\-]+(?:::\w[\w\[\].\-]*)+)\s+(PASSED|FAILED|SKIPPED|ERROR)", re.MULTILINE)
seen, tests = set(), []
...
class BreakerService:
    ...
    def __init__(
        ...
        print(f"Total SWE-bench Pro instances: {self.num_swe_instances}")
    ...
    @staticmethod
    def _generate_expansion_run_script(test_command: str) -> str:
        """Generate a run_script.sh for an expansion (pytest-based) task.

        When called with args (test files), runs those files with pytest.
        When called without args, runs the full test_command from the task.
        """
        return f"""#!/bin/bash
cd /app
if [ $# -eq 0 ]; then
...
"""
    ...
    # ==================== Expansion Task Processing ====================

    def _get_exp_bug_key(self, instance_id: str) -> str:
        return f"{self.r2_prefix}/expansion/{instance_id}.json"

    def _get_exp_claim_key(self, instance_id: str) -> str:
        return f"{self.r2_prefix}/expansion/claims/{instance_id}.claim"

    def exp_bug_exists(self, instance_id: str) -> bool:
        try:
            self.s3.head_object(Bucket=self.r2_bucket, Key=self._get_exp_bug_key(instance_id))
            return True
        except ClientError:
            return False

    def _get_exp_claim(self, instance_id: str) -> Optional[Dict[str, Any]]:
        try:
            response = self.s3.get_object(
                Bucket=self.r2_bucket, Key=self._get_exp_claim_key(instance_id)
            )
            return json.loads(response['Body'].read().decode('utf-8'))
        except ClientError as e:
            if e.response['Error']['Code'] == 'NoSuchKey':
    ...
    def _write_exp_claim(self, instance_id: str) -> None:
        claim_data = {
            "machine_id": self.machine_id,
            "claimed_at": time.time(),
            "expires_at": time.time() + CLAIM_TIMEOUT,
        ...
        self.s3.put_object(
            Bucket=self.r2_bucket,
            Key=self._get_exp_claim_key(instance_id),
            Body=json.dumps(claim_data).encode('utf-8'),
            ContentType='application/json',
        )

    def _release_exp_claim(self, instance_id: str) -> None:
        try:
            claim = self._get_exp_claim(instance_id)
            if claim and claim.get('machine_id') == self.machine_id:
                self.s3.delete_object(
                    Bucket=self.r2_bucket, Key=self._get_exp_claim_key(instance_id)
                )
    ...
    def try_claim_expansion(self, instance_id: str) -> bool:
        """Try to claim an expansion instance for processing."""
        if self.exp_bug_exists(instance_id):
        ...
        self._write_exp_claim(instance_id)
        time.sleep(0.3)  # eventual consistency
        current_claim = self._get_exp_claim(instance_id)
        return bool(current_claim and current_claim.get('machine_id') == self.machine_id)

    def save_expansion_bug(self, instance_id: str, data: Dict[str, Any]) -> bool:
        key = self._get_exp_bug_key(instance_id)
        try:
            self.s3.put_object(
                Bucket=self.r2_bucket,
                Key=key,
            ...
            log(f"[Expansion {instance_id}] ✓ Uploaded to R2: {key}")
            return True
        except Exception as e:
            log(f"[Expansion {instance_id}] ✗ Upload failed: {e}")
            return False

    def list_unprocessed_expansion(self) -> List[Dict[str, Any]]:
        """Scan expansion/ prefix for tasks that don't have a bug yet."""
        bugs_prefix = f"{self.r2_prefix}/expansion/"

        # Collect existing bug instance_ids
        existing_bugs: set = set()
        try:
            paginator = self.s3.get_paginator('list_objects_v2')
            for page in paginator.paginate(Bucket=self.r2_bucket, Prefix=bugs_prefix):
                for obj in page.get('Contents', []):
                    key = obj['Key']
                    if key.endswith('.json') and '/claims/' not in key:
                        existing_bugs.add(key.split('/')[-1][:-5])  # strip .json
        except ClientError:
            pass

        # Load expansion source tasks not yet processed
        src_prefix = f"{EXPANSION_R2_PREFIX}/"
        unprocessed = []
        try:
            paginator = self.s3.get_paginator('list_objects_v2')
            for page in paginator.paginate(Bucket=self.r2_bucket, Prefix=src_prefix):
                for obj in page.get('Contents', []):
                    key = obj['Key']
                    if not key.endswith('.json'):
                        continue
                    instance_id = key.split('/')[-1][:-5]
                    if instance_id in existing_bugs:
                        continue
                    try:
                        response = self.s3.get_object(Bucket=self.r2_bucket, Key=key)
                        inst = json.loads(response['Body'].read().decode('utf-8'))
                        if not inst.get('dockerhub_tag'):
                            continue
                        inst['_source_type'] = 'expansion'
                        unprocessed.append(inst)
                    except Exception as e:
                        log(f"Warning: failed to load expansion instance {key}: {e}")
        except ClientError as e:
            log(f"Warning: could not list expansion instances from R2: {e}")

        return unprocessed

    async def generate_expansion_task(
        self,
        inst: Dict[str, Any],
        max_retries: int = 5,
    ) -> Tuple[bool, Optional[Dict[str, Any]]]:
        """Generate a bug injection for one expansion instance."""
        instance_id = inst["instance_id"]

        if self.exp_bug_exists(instance_id):
            log(f"[Expansion {instance_id}] Already exists, skipping")
            return True, None
        ...
        params = {
            "swe_instance": inst,
            "bug_types": bug_types,
            "seed": hash(instance_id) & 0xFFFFFFFF,
        }

        return await self._try_generate_with_params(
            task_id=instance_id,  # used only for logging
            params=params,
            max_retries=max_retries,
        )

    async def run_expansion(self, max_tasks: Optional[int] = None) -> None:
        """
        Expansion processing loop.

        Continuously scans expansion/ prefix for new tasks, processes them,
        and stores results at bugs/expansion/{instance_id}.json.
        """
        tasks_processed = 0
        tasks_generated = 0
        tasks_failed = 0
        ...
        log("=" * 50)
        log("Starting expansion task processing")
        log(f"Machine ID: {self.machine_id}")
        log("=" * 50)

        while max_tasks is None or tasks_processed < max_tasks:
            unprocessed = self.list_unprocessed_expansion()

            if not unprocessed:
                log("No unprocessed expansion tasks, waiting 60s...")
                await asyncio.sleep(60)
                continue

            log(f"Found {len(unprocessed)} unprocessed expansion tasks")
            ...
            for inst in unprocessed:
                ...
                if not self.try_claim_expansion(instance_id):
                    log(f"[Expansion {instance_id}] Claimed by another worker, skipping")
                    continue

                if self.exp_bug_exists(instance_id):
                    continue

                current_docker_image = inst.get("dockerhub_tag")
                try:
                    success, result = await self.generate_expansion_task(inst)

                    if success and result is not None:
                        result.pop("_docker_image", None)
                        upload_ok = self.save_expansion_bug(instance_id, result)
                        if upload_ok:
                            tasks_generated += 1
                            log(f"[Expansion {instance_id}] ✓ Complete "
                                f"(total: {tasks_generated} generated, {tasks_failed} failed)")
                        else:
                            tasks_failed += 1
                    elif not success:
                        tasks_failed += 1
                        log(f"[Expansion {instance_id}] ✗ Generation failed")

                    tasks_processed += 1
                finally:
                    self._release_exp_claim(instance_id)
                    cleanup_docker_resources(current_image=current_docker_image, max_images=10)

            await asyncio.sleep(5)

        log("=" * 50)
        log(f"Expansion complete: {tasks_processed} processed, "
            f"{tasks_generated} generated, {tasks_failed} failed")
        log("=" * 50)

    # ==================== SWE-bench Pro Task Encoding ====================
    ...
    def _build_breaker_input_expansion(self, params: Dict[str, Any]) -> BreakerInput:
        """Build BreakerInput for an expansion (github-pr) instance."""
        inst = params["swe_instance"]
        test_command = inst.get("test_command", "pytest -v --tb=no")
        fail_to_pass = inst.get("fail_to_pass", [])
        pass_to_pass = inst.get("pass_to_pass", [])
        all_tests = list(set(fail_to_pass) | set(pass_to_pass))

        return BreakerInput(
            docker_image=inst["dockerhub_tag"],
            base_commit=inst["base_commit"],
            repo=inst["repo"],
            instance_id=inst["instance_id"],
            gold_patch=inst["patch"],
            test_cases=all_tests,
            test_runner_script=self._generate_expansion_run_script(test_command),
            test_parser_script=GENERIC_PYTEST_PARSER,
            test_files=inst.get("selected_test_files_to_run", ""),
            ...
            problem_statement_original=inst.get("problem_statement", ""),
            env_cmds="",
            before_repo_set_cmd="",
            temperature=0.7,
            max_iterations=50,
    ...
    def _build_breaker_input(self, params: Dict[str, Any]) -> BreakerInput:
        ...
        # Dispatch for expansion instances
        if swe_inst.get("_source_type") == "expansion":
            return self._build_breaker_input_expansion(params)
    ...
    async def _try_generate_with_params(
        ...
        try:
            ...
            # For expansion instances, store extra source fields so the fixer
            # can verify results without looking up SWE-bench Pro dataset.
            if swe_inst.get("_source_type") == "expansion":
                result_dict["source"].update({
                    "dataset": "github-pr",
                    "docker_image": swe_inst["dockerhub_tag"],
                    "test_command": swe_inst.get("test_command", "pytest -v --tb=no"),
                    "fail_to_pass": swe_inst.get("fail_to_pass", []),
                    "pass_to_pass": swe_inst.get("pass_to_pass", []),
                    "selected_test_files_to_run": swe_inst.get("selected_test_files_to_run", ""),
                })
  ...
  python -m breaker.service &
    ...
    async def _run_all():
        await asyncio.gather(
            ...
            service.run_expansion(max_tasks=args.max_tasks),
        )
