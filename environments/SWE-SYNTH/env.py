...
class SynthActor:
    ...
    def _load_task(self, task_id: Union[int, str]) -> Dict[str, Any]:
        ...
        Args:
            task_id: Numeric task ID (SWE-bench Pro) or instance_id string (expansion)
        ...
        if isinstance(task_id, str):
            bug_instance = self.cache.load_expansion(task_id)
            if bug_instance is None:
                raise ValueError(
                    f"Expansion task '{task_id}' not found. "
    ...
    def _get_docker_image(self, bug_instance: Dict[str, Any]) -> str:
        """Return the Docker image for a bug instance.

        For SWE-bench Pro tasks, derives the image from the instance_id.
        For expansion (github-pr) tasks, the image is stored in source.docker_image.
        """
        source = bug_instance["source"]
        if source.get("dataset") == "github-pr":
            return source["docker_image"]
        return get_dockerhub_image_uri(
            source["swe_instance_id"], self.dockerhub_username, source["repo"]
        )
    ...
    def _verify_fix(
        ...
        # Dispatch expansion (github-pr) tasks to a dedicated path
        if bug_instance.get("source", {}).get("dataset") == "github-pr":
            return self._verify_fix_expansion(bug_instance, fix_patch)
...
"""
    ...
    def _verify_fix_expansion(
        self,
        bug_instance: Dict[str, Any],
        fix_patch: str,
    ) -> tuple[float, Dict[str, Any]]:
        """Verify fix for an expansion (github-pr) task.

        Uses test_command from source directly instead of per-instance scripts.
        """
        import base64
        ...
        test_command = source.get("test_command", "pytest -v --tb=no")
        ...
        # Minimal pytest parser: parse `pytest -v --tb=no` output lines
...
"""
        ...
        gold_patch_b64 = base64.b64encode(gold_patch.encode()).decode('ascii')
        bug_patch_b64 = base64.b64encode(bug_patch.encode()).decode('ascii')
        fix_patch_b64 = base64.b64encode(fix_patch.encode()).decode('ascii')
        parser_b64 = base64.b64encode(parser_script.encode()).decode('ascii')
        entryscript_b64 = base64.b64encode(entryscript.encode()).decode('ascii')
...
"""
        ...
        try:
            print(f"Pulling image for verify_fix: {docker_image}")
            subprocess.run(
            ...
            json_str = stdout[stdout.index(begin_marker) + len(begin_marker):stdout.index(end_marker)].strip()
            output = json.loads(json_str)
            passed_tests = {x["name"] for x in output["tests"] if x["status"] == "PASSED"}
            ...
            test_stats = {
                "target_tests": list(target_tests),
                "target_result": f"{len(target_tests & passed_tests)}/{len(target_tests)}",
                "all_result": f"{len(all_required & passed_tests)}/{len(all_required)}",
                "target_passed": target_passed,
                "all_passed": all_passed,
            }
            if all_passed:
    ...
    async def reset(
        ...
        try:
            ...
            # Get Docker image
            docker_image = self._get_docker_image(bug_instance)
